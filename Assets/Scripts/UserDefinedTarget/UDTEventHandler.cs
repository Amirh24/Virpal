...
public class UDTEventHandler : MonoBehaviour, IUserDefinedTargetEventHandler
{
    ...
    private BuildDialog mBuildDialog;
    private ObjectTracker mObjectTracker;
    private Text mQualityImageUIText;

    private bool mCurrentlyTargeting;
    private Button mNewScanningButton;
    ...
    public void Start()
    {
        ...
        mBuildDialog = FindObjectOfType<BuildDialog>();
        mQualityImageUIText = GameObject.Find("QualityImageText").GetComponent<Text>();
        mNewScanningButton = GameObject.Find("NewScanningButton").GetComponent<Button>();
        mCurrentlyTargeting = false;
        ...
        if (!mBuildDialog)
        {
            mBuildDialog.gameObject.SetActive(true);
        }

            mNewScanningButton.gameObject.SetActive(false);
    ...
    {
        if (!mCurrentlyTargeting)
        {
            mFrameQuality = frameQuality;
            Debug.Log((mFrameQuality + " quality image").Substring(14));
            mQualityImageUIText.text = (mFrameQuality + " quality image").Substring(14).ToLower();
            // Change Text Color
            if (mFrameQuality == ImageTargetBuilder.FrameQuality.FRAME_QUALITY_HIGH)
            {
                // Change text color to green when the quality is good for scanning
                mQualityImageUIText.color = new Color(0, 1, 0, 1);
            }
            else
            {
                // Change text color to red when quality is not good for scanning
                mQualityImageUIText.color = new Color(1, 0, 0, 1);
            }
            Debug.Log(mNewScanningButton);
        }
    }
    ...
    {
        mTargetCounter++;
        mCurrentlyTargeting = (mTargetCounter == 0) ? false : true;
    ...
    public void CloseQBuildDialog()
    {
        if (mBuildDialog)
            mBuildDialog.gameObject.SetActive(false);
    }

    public void ShowNewScanningButton()
    {
        mNewScanningButton.gameObject.SetActive(true);
    }
    ...
    /// <summary>
    /// This method only demonstrates how to handle extended tracking ontSIzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz when you have multiple targets in the scene
